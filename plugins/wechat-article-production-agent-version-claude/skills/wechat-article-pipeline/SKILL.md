...
description: Primary and only external entrypoint for converting a completed Chinese podcast narration file, especially narration.txt, into a WeChat Official Account article draft. This agent-version workflow uses fixed Markdown contracts (article_draft.md + image_candidates.md), deterministic JSON conversion via scripts, package validation, rendering, and optional draft upload.
metadata:
  short-description: 口播稿到公众号草稿，Markdown 契约 + 脚本确定性解析
---
...
You own the entire workflow: user communication, input/output directories, credential checks, image searching and downloading, Markdown contract delivery, deterministic script execution, upload decisions, and final reporting.

JSON (`article.json`, `image_manifest.json`) is an internal script interface only. You must write Markdown contracts, not directly hand-write final JSON files.
...
```text
narration.txt
→ you create article_dir
→ you write article_draft.md + image_candidates.md + images/
→ you run run_wechat_article_package.py --article-dir ...
→ scripts parse Markdown into JSON
→ scripts prepare images, validate, render article.html, write preflight
→ you upload only when user requests and credentials are available
```
...
`batch_state.json` is maintained by you. Allowed statuses are `assigned`, `draft_ready`, `validated`, `rendered`, `uploaded`, and `failed`.

## Delivery Contract

For each article, you read the narration, craft the article, search and judge image value, and download reliable local images. You must deliver exactly these Markdown contract files:
...
You must not directly create final JSON files (`article.json`, `image_manifest.json`). Those are generated by scripts from your Markdown contracts.
...
At upload time, read existing environment variables first, then `~/.claude/wechat.env`:
...
- You delivered only Markdown contracts and local images, not final JSON.
